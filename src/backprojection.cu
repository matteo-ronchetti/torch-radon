...
template<bool extend, int channels>
__global__ void
...
void radon_backward_cuda(const float *x, const float *rays, const float *angles, float *y, TextureCache &tex_cache,
    ...
    Texture *tex = tex_cache.get({device, batch_size, n_rays, n_angles, channels, PRECISION_FLOAT});
    tex->put(x);
...
//    const int wpt = (batch_size % 16 == 0) ? 4 : 1;
    ...
    // Invoke kernel
    if (extend) {
        radon_backward_kernel_half<true, 1> << < dimGrid, dimBlock >> >
                                                          ((__half *) y, tex->texture, rays, angles, img_size, n_rays, n_angles);
    } else {
        radon_backward_kernel_half<false, 1> << < dimGrid, dimBlock >> >
                                                           ((__half *) y, tex->texture, rays, angles, img_size, n_rays, n_angles);
    }
}
