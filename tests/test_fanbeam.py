            ...
            for spacing in [1.0, 0.5, 1.3, 2.0]:
                for distances in [(1.2, 1.2), (2.0, 2.0), (1.2, 3.0)]:
                    for clip_to_circle in [False, True]:
                        params.append((device, batch_size, image_size, angles, spacing, distances, clip_to_circle))

half_params = [x for x in params if x[1] % 4 == 0]


@parameterized(params)
def test_fanbeam_error(device, batch_size, image_size, angles, spacing, distances, clip_to_circle):
    # generate random images
    x = generate_random_images(1, image_size, masked=clip_to_circle)[0]
    ...
    proj_geom = astra.create_proj_geom('fanflat', spacing, x.shape[0], angles, s_dist, d_dist)
    ...
    # our implementation
    radon = RadonFanbeam(image_size, angles, s_dist, d_dist, det_spacing=spacing, clip_to_circle=clip_to_circle)
...
def test_half(device, batch_size, image_size, angles, spacing, distances, clip_to_circle):
    # generate random images
