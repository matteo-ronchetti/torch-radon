...
with torch.no_grad():
    ...
    y_ = radon_noise.emulate_readings(y, 5.0, 50.0)
    read_mean = torch.zeros((256,)).to(device)
    z = radon.readings_lookup(y_, read_mean)
...
plt.imshow(y_[0], cmap="gray")
plt.figure()
plt.imshow(z[0].cpu().numpy(), cmap="gray")
plt.show()
# print(z.size())
# z *= np.pi / n_angles
# # a_rec /= np.max(a_rec)
